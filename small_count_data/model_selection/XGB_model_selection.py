...
for train_ix, test_ix in cv_outer.split(X,y ):
    ...
    ++i   
    if i == 1 :
        with open("../../../../ML_results/small_gene_count/hyper_param_tunning/xgb_tunning_process.txt" , 'w') as f:
            for ele in  [" Baseline model accuracy" , str( max(y_test.mean(), 1 - y_test.mean()) ) ]:
                f.write(str(ele))
    else :
        with open("../../../../ML_results/small_gene_count/hyper_param_tunning/xgb_tunning_process.txt" , 'a') as f:
            f.write('\n')
            for ele in  [" Baseline model accuracy" , str( max(y_test.mean(), 1 - y_test.mean()) ) ]:
                f.write(str(ele))
